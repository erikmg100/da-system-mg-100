            ...
            while (attempt <= maxRetries) {
                try {
                    ...
                    const { error } = await supabase
                        ...
                        .eq('call_id', callId);
                    if (error) {
                        console.error(`Supabase update attempt ${attempt} failed:`, error);
                        if (attempt === maxRetries) {
                            console.error('Max retries reached for Supabase update');
                            throw error;
                        }
                    } else {
            ...
            try {
                ...
                if (response.type === 'input_audio_buffer.speech_started') {
                    if (silenceTimer) {
                        ...
                        silenceTimer = null;
                        console.log('Speech detected, silence timer cleared');
                    }
                    handleSpeechStartedEvent();
                }
                if (response.type === 'input_audio_buffer.speech_stopped') {
                    if (!silenceTimer) {
                        console.log('Speech stopped, starting 3-second silence timer');
                        silenceTimer = setTimeout(() => {
                            if (conversationWs && conversationWs.readyState === WebSocket.OPEN) {
                                console.log('Silence detected for 3 seconds, sending reminder');
                                const reminderItem = {
                                    type: 'conversation.item.create',
                                    item: {
                                        type: 'message',
